...
const typeConfig: Record<string, { icon: React.ReactNode; bg: string; color: string }> = {
  LAPTOP:  { icon: <Laptop  size={20} />, bg: "rgba(96,144,227,0.1)",  color: "#6090E3" },
  MONITOR: { icon: <Monitor size={20} />, bg: "rgba(124,58,237,0.1)",  color: "#7c3aed" },
};
    ...
    <UserShell userName="Alex">
      <div className="space-y-5 max-w-4xl">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total Assigned", value: userAssets.length.toString(), color: "#6090E3", bg: "rgba(96,144,227,0.08)", border: "rgba(96,144,227,0.15)" },
            { label: "Active",         value: userAssets.filter((a) => a.status === "ACTIVE").length.toString(),    color: "#12B76A", bg: "rgba(18,183,106,0.08)", border: "rgba(18,183,106,0.15)" },
            { label: "Under Repair",   value: userAssets.filter((a) => a.status === "REPAIR").length.toString(),    color: "#ef4444", bg: "rgba(239,68,68,0.08)",  border: "rgba(239,68,68,0.15)"  },
          ].map(({ label, value, color, bg, border }) => (
            <div key={label} className="rounded-2xl p-4" style={{ background: bg, border: `1px solid ${border}` }}>
              <p className="text-[11px] font-bold uppercase tracking-[0.08em] mb-1.5" style={{ color }}>{label}</p>
              <p className="text-[1.6rem] font-bold leading-none" style={{ color }}>{value}</p>
            </div>
          ))}
        </div>
        ...
        {userAssets.length > 0 ? (
          <div className="grid grid-cols-2 gap-4">
            {userAssets.map((asset) => {
              const tc = typeConfig[asset.type] ?? { icon: <Package size={20} />, bg: "rgba(100,116,139,0.1)", color: "#64748b" };
              return (
                <div
                  key={asset.id}
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 4px 16px rgba(0,0,0,0.08)"; e.currentTarget.style.border = "1px solid #d4e0f0"; }}
                ...
                >
                  ...
                  >
                    <button
                      className="flex items-center gap-1.5 text-[12px] font-semibold text-[#5a7090] transition-colors duration-150 hover:text-primary"
                    >
                      <HelpCircle size={13} /> Request Support
                    </button>
                    <button
                      className="flex items-center gap-1.5 text-[12px] font-semibold text-[#5a7090] transition-colors duration-150 hover:text-primary"
                    >
          ...
          >
            <div
              ...
              style={{ background: "rgba(96,144,227,0.1)" }}
            >
